...
export function DepartmentCard({ dept }: Props) {
  ...
  return (
    <div className={`rounded-lg border-l-4 ${DEPT_ACCENTS[id]} ${DEPT_BG[id]} p-4`}>
      ...
      {/* Staff section */}
      <div className="space-y-1 text-sm mb-3">
        <div className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Staff</div>
        ...
        <div className="flex justify-between text-xs">
          ...
          <span className={cIdle > 0 ? 'text-green-400' : 'text-gray-500'}>{cIdle}</span>
        </div>
        ...
        {(dept.staff.extra_total > 0 || dept.staff.extra_incoming > 0) && (
          <>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className={eIdle > 0 ? 'text-blue-400' : 'text-gray-500'}>{eIdle}</span>
            </div>
          </>
        )}
      ...
      {(dept.arrivals_waiting > 0 || reqWaiting > 0) && (
        <div className="space-y-1 text-sm mb-3">
          <div className="text-gray-400 text-xs font-medium uppercase tracking-wide mb-1">Waiting</div>
          {dept.arrivals_waiting > 0 && (
            <div className="flex justify-between text-xs">
              <span className="text-gray-500 pl-2">Walk-in arrivals</span>
              <span className="text-yellow-400">{dept.arrivals_waiting}</span>
            </div>
          )}
          {Object.entries(dept.requests_waiting).map(([fromDept, count]) => {
            ...
            return (
              <div key={fromDept} className="flex justify-between text-xs">
